...
- 🎯 **智能去重**：基于 OpenAlex ID 自动去重，避免重复推送相同论文
- ⏰ **延迟优化**：使用 `created_date` 过滤，覆盖 OpenAlex 同步延迟（默认获取最近 48 小时内的论文）
...
- 查询关键词：`LLM`, `agent`, `language model`, `safety`, `security`, `robustness`
- 获取数量：50 篇
- 推送策略：
  - 满分论文（5 分）：全部推送
  - 其他高分论文（3-4.9 分）：最多推送 20 篇
  - 低分论文（< 3 分）：不推送
- 时间范围：最近 4 天（覆盖周末，ArXiv 周末不更新）
- 运行时间：每天北京时间 22:00（晚 10 点）
...
4. **定时运行**
   ...
   工作流默认每天 UTC 14:00（北京时间 22:00，晚 10 点）自动运行

   如需修改时间，编辑 [.github/workflows/arxiv-daily.yml](.github/workflows/arxiv-daily.yml) 中的 `cron` 表达式

> 注意：GitHub Actions 的 cron 使用 UTC 时间；如需改运行时间，编辑 `.github/workflows/arxiv-daily.yml` 里的 `cron`。

5. **去重机制**

   项目使用 `pushed_ids.txt` 文件记录已推送的论文 ID，避免重复推送：
   - 每次运行时自动检查并过滤已推送的论文
   - GitHub Actions 会自动提交更新后的 `pushed_ids.txt`
   - 文件只保留最近 500 条记录，避免过大
